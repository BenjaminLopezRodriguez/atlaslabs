...
var DEFAULT_BASE = process.env.ATLAS_BASE_URL ?? "https://www.atlaslabs.id";
function configDir() {
...
function installationId() {
  ...
  const generated = (0, import_node_crypto.randomUUID)();
  fs.mkdirSync(configDir(), { recursive: true });
...
var KEYCHAIN_SERVICE = "id.atlaslabs.cli";
function saveToken(token) {
  ...
  fs.mkdirSync(configDir(), { recursive: true });
  fs.writeFileSync(path.join(configDir(), "token"), token, { mode: 384 });
}
function loadToken() {
  if (process.platform === "darwin") {
    try {
...
async function api(method, route, body, opts = { auth: true }) {
  const headers = {
  ...
  if (!res.ok) {
    ...
    const detail = json?.rejected?.map(
      (r) => `  ${r.path} \u2014 ${r.reason}`
    ).join("\n") ?? "";
    fail(
      `${method} ${route} failed (${res.status}): ${json?.error ?? "error"}` + (json?.hint ? `
${json.hint}` : "") + (detail ? `
${detail}` : "")
    );
  }
...
function readAtlasYaml() {
  try {
    return (0, import_yaml.parse)(fs.readFileSync(ATLAS_YAML, "utf8"));
  } catch {
    return null;
  }
}
...
async function resolveGroupId(slug) {
  ...
  const g = groups.find((x) => x.slug === slug || x.id === slug);
...
`);
  ...
  while (Date.now() < deadline) {
    await new Promise((r) => setTimeout(r, code.interval * 1e3));
...
async function cmdLogout() {
  if (loadToken()) {
    await api("DELETE", "/api/v1/cli/token").catch(() => void 0);
  }
  deleteToken();
...
async function cmdGroup(sub, rest) {
  if (sub === "list" || !sub) {
    const { groups } = await api("GET", "/api/v1/cli/groups");
...
function cmdInit() {
  ...
  fs.writeFileSync(ATLAS_YAML, (0, import_yaml.stringify)(cfg));
...
async function cmdLink() {
  ...
  if (yamlCfg.specialist) {
    ...
    const sp = specialists.find((s) => s.slug === yamlCfg.specialist);
    console.log(
      sp ? `Specialist: ${sp.name} (${sp.state})` : `Specialist "${yamlCfg.specialist}" not found in workspace.`
    );
  }
}
...
async function cmdSource(sub, rest) {
  if (sub === "add") {
    const p = rest[0];
...
Source ${src.path} \u2192 workspace ${ws.label}`);
      ...
      const origin = `path:${name}:${(0, import_node_crypto.createHash)("sha256").update(root).digest("hex").slice(0, 12)}`;
      ...
      console.log(
        res.unchanged ? `  Unchanged (v${res.version}).` : `  Synced v${res.version} (${res.fileCount} files).`
      );
    }
    return;
  }
...
async function cmdSpecialist(sub, rest) {
  ...
  if (sub === "inspect") {
    ...
    const sp = specialists.find((s) => s.slug === slug || s.id === slug);
...
async function findSpecialist(workspaceId, slugOrId) {
  const { specialists } = await api(
    "GET",
...
async function cmdApiKey(sub, rest) {
...
function promptYesNo(q) {
  ...
  return new Promise((resolve2) => {
    ...
    process.stdin.once("data", (d) => {
      process.stdin.pause();
      resolve2(/^y(es)?$/i.test(String(d).trim()));
    });
    process.stdin.resume();
  });
}
function parsePingQuestion(rest) {
  ...
  return rest.filter((a, i) => a !== "--no-wait" && !consumed.has(i)).join(" ").trim();
}
async function cmdPingUser(slug, rest) {
  if (!slug) {
  ...
  const res = await api("POST", `/api/v1/machines/${m.id}/ping`, {
    question,
    context: contextIdx === -1 ? void 0 : rest[contextIdx + 1],
...
`);
  ...
  while (Date.now() < deadline) {
    ...
    delay = Math.min(delay * 1.4, 1e4);
    const { ping } = await api("GET", `/api/v1/pings/${res.ping.id}`);
...
async function cmdPingLog(slug) {
  if (!slug) fail("Usage: atlas ping log <slug>");
  const m = await findMachine(slug);
  const { pings } = await api("GET", `/api/v1/machines/${m.id}/ping`);
...
async function cmdMachine(sub, rest) {
  if (sub === "list" || sub === "ls") {
    const w = await resolveWorkspace();
...
function parseExecArgs(rest) {
  const sep2 = rest.indexOf("--");
  return sep2 === -1 ? rest : rest.slice(sep2 + 1);
}
async function cmdExec(slug, rest) {
...
async function cmdPut(slug, rest) {
...
async function cmdGet(slug, rest) {
...
async function cmdPorts(slug) {
  if (!slug) fail("Usage: atlas ports <slug>");
  const m = await findMachine(slug);
  const { ports } = await api("GET", `/api/v1/machines/${m.id}/ports`);
  if (!ports.length) return console.log("No ports.");
  for (const p of ports) {
    console.log(`${p.port}${p.label ? `  ${p.label}` : ""}`);
  }
}
async function cmdDevice(sub, rest) {
  if (sub === "list" || sub === "ls" || sub === void 0) {
    const { devices } = await api("GET", "/api/v1/devices");
    if (!devices.length) return console.log("No devices.");
    for (const d of devices) {
      const seen = d.lastSeenAt ? new Date(d.lastSeenAt).toISOString().slice(0, 16).replace("T", " ") : "never";
      const flags = [
...
var ENTRY_NAMES = ["cli.ts", "cli.js", "atlas.cjs", "atlas.js", "atlas"];
var invokedAs = process.argv[1]?.split(/[\\/]/).pop() ?? "";
var isDirectRun = ENTRY_NAMES.includes(invokedAs);
if (isDirectRun) {
